...
axios.interceptors.request.use(config => {
  ...
  config.headers[settings.LOGIN_UUID_KEY] = localStorage.getItem(settings.LOGIN_UUID_KEY)
  config.headers[settings.LOGIN_GH_KEY] = localStorage.getItem(settings.LOGIN_GH_KEY)
  // 在最后必须return config
  return config
})
...
axios.interceptors.response.use(function (res) {
  if (res.data.code === 10000) { // 登录过期
    ElementUI.Message.error('登录过期，请重新登录!')
    router.push('/login')
  }
  if (res.data.code === 10001) { // 统一异常提示
